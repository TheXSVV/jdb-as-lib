                        ...
                        if (target != -1) {
                            ...
                            if ((len = is.read(payload)) > -1) {
                                ...
                                if (payloadLen >= target) {
                                    DataInputStream in = new DataInputStream(new ByteArrayInputStream(accumulator.toByteArray()));
                                    int id = in.readInt();
                                    ...
                                    switch (id) {
                                        ...
                                        case 1: // resp init
                                            break;
                                        default:
                                            System.err.println("INVALID SIGNAL: " + id);
                                    }
                                    ...
                                    while (in.available() > 0) {
                                        if (headerLen < 4) {
                                            headerLen += 1;
                                            headerAccum.write(in.readByte());
                                        } else
                                            accumulator.write(in.readByte());
                                    }
                                }
                            } else {
                                break;
